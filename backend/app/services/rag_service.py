...
class RagService:
    ...
    - GitHub org/user URL (indexes multiple repos)
    """
    ...
    def _resolve_repo_paths(self) -> dict[str, Path]:
        ...
        if parsed.scheme in {"http", "https"} and parsed.netloc:
            ...
            target_dir = cache_dir / "default_repo"
            ...
            return self._with_catalog_paths({"default_repo": target_dir})
    ...
    def _clone_github_org_repos(self, source: str) -> dict[str, Path]:
        ...
        if not repos:
            raise RuntimeError(f"No repositories discovered for GitHub owner '{owner}'")
    ...
    def _fetch_github_repos(
        ...
        for url in endpoints:
            try:
                return self._fetch_paginated_repo_list(url, headers)
    ...
    def _extract_next_link(self, link_header: str | None) -> str | None:
        ...
        for p in parts:
            ...
            if lt == -1 or gt == -1:
                continue
            url = p[lt + 1 : gt]
            # Normalize empty query fragments to avoid malformed repeats.
            parsed = urlsplit(url)
            if not parsed.query:
                return url
