...
from app.models.schemas import AnalyticsSummary, HistoryEntry, HistoryResponse
...
# Token estimation: rough — 1 token ~ 4 chars on average for English.
CHARS_PER_TOKEN = 4


class AnalyticsService:
    """Tracks aggregate stats + recent history. SRP: read/write analytics
    state in Redis only."""

    def __init__(self, qdrant: QdrantInfrastructure, redis_infra: RedisInfrastructure):
        self._qdrant = qdrant
    ...
    async def record_query(
        ...
        # Estimate time saved: cache_hit avoided an LLM call.
        avg_llm_latency = await self._get_avg_llm_latency()
        if (
            avg_llm_latency > 0
            and agent_action == AgentAction.CACHE_HIT
        ):
    ...
    async def summary(self) -> AnalyticsSummary:
        ...
        try:
            cache_entries = (await self._qdrant.client.count(
                self._qdrant._settings.qdrant_collection
            )).count
        except Exception:
            cache_entries = 0
